...
function start() {
    ...
    var updateGr4vy; 
    if(gr4vyPreferences.isGr4vyEnabled() && request.httpParameterMap.isGr4vyOrder.value) {
        updateGr4vy = gr4vyHelper.updateBuyerInGr4vy(currentBasket);
    ...
    //Gr4vy-Begin
    var res = require('*/cartridge/scripts/util/Response');
    if(request.httpParameterMap.isGr4vyOrder.value && (handlePaymentsResult.error || handlePaymentsResult.missingPaymentInfo)) {
        Transaction.wrap(function(){
    ...
    if(gr4vyPreferences.isGr4vyEnabled() && request.httpParameterMap.isGr4vyOrder.value) {
        ...
        res.renderJSON({
            ...
            continueUrl: URLUtils.url('Gr4vy-HandleGr4vyRedirect').toString()
        });
        return;
    } else {   
