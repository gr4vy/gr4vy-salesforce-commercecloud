...
function start(cart, params) {
    ...
    // Gr4vy - Begin
    // sets the viewData with gr4vy data
    if (!!currentBasket && gr4vyPreferences.isGr4vyEnabled()) {
        params.selectedPaymentID = Constants.GR4VY_ID;
        params = gr4vyHelper.updateViewDataWithGr4vyData(currentBasket, params, customerData);
    }
    // Gr4vy - End
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            ...
            Transaction.wrap(function () {
                ...
                var isGr4vyEnabled = gr4vyPreferences.isGr4vyEnabled();
                var cart = app.getModel('Cart').get();
                if (!cart) {
                    res.renderJSON({
                        cartError: true,
                        continueURL: URLUtils.https('Cart-Show').toString()
                    });
                    return;
                }
                //Gr4vy-End
                ...
                handlePaymentSelection(cart).error) {// Performs payment method specific checks, such as credit card verification.
                    //Gr4vy-Begin:
                    var isFormvalid = (request.httpParameterMap.isFormvalid.stringValue == 'true');
                    if (isGr4vyEnabled) {
                        if(isFormvalid) {
                            res.renderJSON({
                    ...
                    //Gr4vy-Begin: 
                    if (!isGr4vyEnabled || request.httpParameterMap.dwfrm_billing_paymentMethods_selectedPaymentMethodID.stringValue !== Constants.GR4VY_ID) {
